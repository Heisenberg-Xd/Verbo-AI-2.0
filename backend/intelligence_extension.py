...
def register_intelligence(app: FastAPI):
    ...
    @app.post("/workspace/{workspace_id}/connect-drive")
    ...
    @app.get("/workspace/{workspace_id}/drive-status")
    ...
    @app.get("/workspace/{workspace_id}/entities")
    ...
    @app.post("/workspace/{workspace_id}/entities/refresh")
    ...
    @app.get("/workspace/{workspace_id}/relationships")
    ...
    @app.get("/workspace/{workspace_id}/knowledge-graph")
    ...
    @app.get("/workspace/{workspace_id}/entity/{entity_name}/connections")
    async def entity_connections(workspace_id: str, entity_name: str):
        ...
        result = get_entity_connections(entity_name, entities, relationships)
    ...
    @app.get("/workspace/{workspace_id}/entities/search")
    async def search_entities(workspace_id: str, q: str, limit: int = 20):
    ...
    # ══════════════════════════════════════════════════════════════
    # 7. WORKSPACE-AWARE DOCUMENT INGESTION HOOK
    # ══════════════════════════════════════════════════════════════

    @app.post("/workspace/{workspace_id}/process")
    async def workspace_process(workspace_id: str, background_tasks: BackgroundTasks):
        """
        Trigger entity/relationship extraction on existing workspace documents.
        Does NOT re-run the main clustering pipeline — that stays in /process.
        Call this after running /process to enrich workspace intelligence.
        """
        ws = get_workspace(workspace_id)
        if not ws:
            raise HTTPException(status_code=404, detail="Workspace not found.")
        ...
        if not doc_texts:
            raise HTTPException(
                status_code=400,
                detail="Workspace has no documents. Upload files and run /process first, then attach workspace_id.",
            )

        # Run in background
        background_tasks.add_task(_run_pipeline_sync, workspace_id, doc_texts)

        return JSONResponse({
            "status":      "processing",
            "workspace_id": workspace_id,
            "message":     "Entity and relationship extraction started in background.",
        })

    # ══════════════════════════════════════════════════════════════
    # 8. ENTITY-ENRICHED CHAT CONTEXT ENDPOINT
    # ══════════════════════════════════════════════════════════════

    @app.post("/workspace/{workspace_id}/chat/context")
    async def get_enriched_context(workspace_id: str, req: EnrichChatRequest):
        """
        Returns entity + relationship context to enrich a RAG chat query.
        The frontend can merge this with /rag/chat output for richer answers.
        """
        ws = get_workspace(workspace_id)
        if not ws:
            raise HTTPException(status_code=404, detail="Workspace not found.")
        ...
        mentioned_names = {e["name"].lower() for e in mentioned_entities}
        relevant_rels = [
            r for r in relationships
            if r["subject"].lower() in mentioned_names
            or r["object"].lower() in mentioned_names
        ]

        # Build mini knowledge graph for this query
        mini_graph = build_knowledge_graph(
            mentioned_entities or entities[:20],
            relevant_rels or relationships[:20],
        )
        ...
        return JSONResponse({
            ...
            "context_summary": _build_context_summary(mentioned_entities, relevant_rels),
        })

    # ══════════════════════════════════════════════════════════════
    # 9. WORKSPACE STATS DASHBOARD
    # ══════════════════════════════════════════════════════════════

    @app.get("/workspace/{workspace_id}/stats")
    async def workspace_stats(workspace_id: str):
        """Aggregated intelligence stats for a workspace dashboard."""
        ws = get_workspace(workspace_id)
        if not ws:
            raise HTTPException(status_code=404, detail="Workspace not found.")
        ...
        type_counts = get_entity_summary(entities)
        rel_type_counts: dict = {}
        for r in relationships:
            t = r.get("relationship", "unknown")
            rel_type_counts[t] = rel_type_counts.get(t, 0) + 1

        # Top entities by source file count
        top_entities = sorted(
            entities,
            key=lambda e: len(e.get("source_files", [])),
            reverse=True,
        )[:10]
    ...
    print("[VerboAI] ✅ Intelligence extension registered — Workspace, Drive, Entities, Relationships, Knowledge Graph")


# ─────────────────────────────────────────────
# HELPERS
# ─────────────────────────────────────────────
def _build_context_summary(entities: list, relationships: list) -> str:
    """Build a short natural-language context string for AI chat enrichment."""
    parts = []
    if entities:
        names = [e["name"] for e in entities[:5]]
        parts.append(f"Relevant entities: {', '.join(names)}.")
    if relationships:
        rels = [f"{r['subject']} {r['relationship'].replace('_', ' ')} {r['object']}"
                for r in relationships[:5]]
        parts.append(f"Known relationships: {'; '.join(rels)}.")
    return " ".join(parts) if parts else "No entity context found for this query."