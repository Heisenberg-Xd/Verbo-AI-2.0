...
const safeGet = async (url, opts) => {
  try { return await axios.get(url, opts); }
    ...
    if (e.code === "ERR_NETWORK" || e.message === "Network Error") {
      const ce = new Error(`CORS_OR_SERVER: Backend error on ${url} — check FastAPI terminal.`);
...
const parseCorsError = (e, fallback) => {
  if (e?.isCorsError) return `Backend error on ${e.url} — CORS header stripped. Open FastAPI terminal for traceback.`;
...
function KnowledgeGraphCanvas({ entities, relationships }) {
  ...
  useEffect(() => {
    ...
    const nodeList = entities.slice(0, 25).map((e, i) => {
      ...
      const isMain = i === 0;
      return { id: name, label: name, type, size: isMain ? 34 : 24, x: W/2 + Math.cos(angle)*r, y: H/2 + Math.sin(angle)*r, vx: 0, vy: 0, dragging: false };
    });
    ...
    let hovered = null, dragNode = null, dragOX = 0, dragOY = 0;
    ...
    function drawIcon(ctx, n, r) {
      ...
      if (n.type === "PERSON") {
        ...
        ctx.fillStyle = KG_COLORS[n.type]?.fill || "#64748b";
    ...
    function draw() {
      ...
      edgeList.forEach(e => {
        ...
        ctx.lineWidth   = hi ? 1.6 : 0.8; ctx.stroke();
        if (hi && e.label) {
          ctx.font = "400 9px monospace"; ctx.fillStyle = "rgba(180,190,210,0.7)";
      ...
      nodeList.forEach(n => {
        ...
        if (isH && n.size >= 30) {
          ...
          const ty = n.y-th/2;
          ctx.fillStyle = "rgba(12,18,28,0.96)"; ctx.strokeStyle = "rgba(255,255,255,0.16)";
          ...
          ctx.fillText(n.label.length > 16 ? n.label.slice(0,15)+"…" : n.label, tx+10, ty+17);
    ...
    const onMove  = e => { const {x,y} = getPos(e); hovered = getNode(x,y); canvas.style.cursor = hovered ? "pointer" : "grab"; if (dragNode) { dragNode.x = x-dragOX; dragNode.y = y-dragOY; } };
    const onDown  = e => { const {x,y} = getPos(e); const n = getNode(x,y); if (n) { dragNode=n; n.dragging=true; dragOX=x-n.x; dragOY=y-n.y; canvas.style.cursor="grabbing"; } };
  ...
  const typeCounts = entities.reduce((acc, e) => {
    ...
    const t = rawType.includes("person")||rawType==="per" ? "PERSON" : rawType.includes("org") ? "ORG" : rawType.includes("loc")||rawType==="gpe" ? "LOC" : "OTHER";
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", gap:14, padding:"8px 12px", background:"rgba(255,255,255,0.03)", borderRadius:"8px 8px 0 0", borderBottom:"1px solid rgba(255,255,255,0.07)", flexWrap:"wrap" }}>
        {[{type:"PERSON",label:"People",count:typeCounts.PERSON},{type:"ORG",label:"Organizations",count:typeCounts.ORG},{type:"LOC",label:"Locations",count:typeCounts.LOC},{type:"OTHER",label:"Other",count:typeCounts.OTHER}]
      ...
      <canvas ref={canvasRef} style={{ width:"100%", background:"#0d1117", borderRadius:"0 0 10px 10px", display:"block", cursor:"grab" }} height={440} />
...
export default function UploadPage({ onComplete }) {
  ...
  // ── FIX: workspaceId is the single source of truth ─────────────────────────
  // It is set ONCE during handleUpload and never changed unless user types a new one.
  // All intelligence functions use it directly — they never call ensureWorkspace().
  const [workspaceId,       setWorkspaceId]        = useState("");
  const [workspaceInput,    setWorkspaceInput]     = useState(""); // what the user types

  // ── Intelligence / Entity state ─────────────────────────────────────────────
  ...
  const handleFileChange = e => {
    ...
    setFiles(sel); setError(null);
    setFileThumbs(sel.map(f => f.type.startsWith("image/") ? URL.createObjectURL(f) : null));
  };

  // ─────────────────────────────────────────────────────────────────────────────
  // FIX: ensureWorkspace — called ONLY from handleUpload.
  // Creates workspace once, stores the real ID in both workspaceId state
  // and returns it for immediate use in the same upload call.
  // ─────────────────────────────────────────────────────────────────────────────
  const ensureWorkspace = async () => {
    const inputId = workspaceInput.trim();

    // If user typed an ID, use it as-is (they may be resuming a session)
    if (inputId) {
    ...
    try {
      ...
      // Backend unavailable — use name as fallback ID
      setWorkspaceId(name);
      return name;
    }
  };
  ...
  const handleUpload = async () => {
    ...
    // Reset intelligence state from any previous run
    setEntities(null); setRelationships(null); setKnowledgeGraph(null);

    const fd = new FormData();
    files.forEach(f => fd.append("files", f));

    try {
      // ── Step 1: get/create workspace ONCE ────────────────────────────────
      ...
      if (d.rag_ready) {
        setChatMessages([{
          ...
          content: `Knowledge base indexed — **${d.rag_chunks_indexed ?? "?"}** chunks across **${Object.keys(d.clusters || {}).length}** clusters. Ask me anything about your documents.`,
        }]);
      }
    } catch (e) {
      setError(parseCorsError(e, "Processing failed. Check backend is running on port 8000."));
      console.error(e);
    } finally { setLoading(false); }
  };

  // ─────────────────────────────────────────────────────────────────────────────
  // FIX: Intelligence functions use workspaceId directly — no ensureWorkspace().
  // This guarantees they always operate on the workspace that was created during
  // handleUpload, never accidentally creating a new empty workspace.
  // ─────────────────────────────────────────────────────────────────────────────

  const runEntityExtraction = async () => {
    if (!clusters) return;
    if (!workspaceId) {
      setEntities({ error: "No workspace linked. Upload and process documents first." });
      return;
    }
    setEntityLoading(true); setEntities(null);
    try {
      await safePost(`${BASE}/workspace/${workspaceId}/entities/refresh`);
      const full = await safeGet(`${BASE}/workspace/${workspaceId}/entities`);
      setEntities(full.data);
    } catch (e) {
      setEntities({ error: parseCorsError(e, "Entity extraction failed.") });
  ...
  const runRelationshipExtraction = async () => {
    if (!clusters) return;
    if (!workspaceId) {
      setRelationships({ error: "No workspace linked. Upload and process documents first." });
      return;
    }
    setEntityLoading(true); setRelationships(null);
    try {
      // Ensure entities exist before extracting relationships
      await safePost(`${BASE}/workspace/${workspaceId}/entities/refresh`);
      const res = await safeGet(`${BASE}/workspace/${workspaceId}/relationships`);
      setRelationships(res.data);
    } catch (e) {
      setRelationships({ error: parseCorsError(e, "Relationship extraction failed.") });
    } finally { setEntityLoading(false); }
  };
  ...
  const buildKnowledgeGraph = async () => {
    if (!clusters) return;
    if (!workspaceId) {
      setKnowledgeGraph({ error: "No workspace linked. Upload and process documents first." });
      return;
    }
    setEntityLoading(true); setKnowledgeGraph(null);
    try {
      await safePost(`${BASE}/workspace/${workspaceId}/entities/refresh`);
      const res = await safeGet(`${BASE}/workspace/${workspaceId}/knowledge-graph`);
      setKnowledgeGraph(res.data);

      // Also refresh entities + relationships for the graph canvas
      const entFull = await safeGet(`${BASE}/workspace/${workspaceId}/entities`);
      setEntities(entFull.data);
      try {
        const relFull = await safeGet(`${BASE}/workspace/${workspaceId}/relationships`);
        setRelationships(relFull.data);
      } catch {}
    } catch (e) {
      setKnowledgeGraph({ error: parseCorsError(e, "Knowledge graph build failed.") });
    } finally { setEntityLoading(false); }
  };
  ...
  const ingestDrive = async () => {
    ...
    try {
      ...
      setDriveResult({ error: e?.response?.data?.detail || "Drive ingestion failed." });
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      if (ans.startsWith("__no_llm__") || ans.includes("ANTHROPIC_API_KEY") || ans.includes("GEMINI_API_KEY")) {
        ans = "⚠️ LLM API key not detected. Check your `.env` file and restart the backend.";
      }
      setChatMessages(p => [...p, { role: "assistant", content: ans, sources: src, cluster_scope: d.cluster_scope }]);
    } catch {
      setChatError("Chat request failed — check the backend console.");
  ...
  // FIX: intelligence buttons are disabled until workspaceId is set
  const intelDisabled = entityLoading || !clusters || !workspaceId;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .grid-bg { position:fixed; inset:0; z-index:0; pointer-events:none; background-image:radial-gradient(circle at 20% 20%, rgba(255,140,0,0.08), transparent 40%), radial-gradient(circle at 80% 0%, rgba(255,180,0,0.05), transparent 35%); }
        ...
        .scard { background:rgba(18,18,23,0.85); backdrop-filter:blur(10px); border:1px solid var(--line); border-radius:var(--r3); padding:16px 20px; position:relative; overflow:hidden; transition:all 0.25s ease; }
        ...
        .scard:hover { border-color:rgba(255,140,0,0.3); transform:translateY(-2px); }
        .sc-v { --c:rgba(255,179,0,0.45); } .sc-t { --c:rgba(251,191,36,0.45); } .sc-g { --c:rgba(255,140,0,0.45); } .sc-a { --c:rgba(255,179,0,0.45); }
        ...
        .panel { background:rgba(18,18,23,0.85); backdrop-filter:blur(10px); border:1px solid var(--line); border-radius:var(--r4); overflow:hidden; }
        ...
        .dropzone:hover .drop-ico { background:rgba(255,140,0,0.18); border-color:rgba(255,140,0,0.4); transform:scale(1.06); }
        ...
        .ws-badge { display:inline-flex; align-items:center; gap:6px; margin-top:6px; padding:4px 10px; background:rgba(255,140,0,0.07); border:1px solid rgba(255,140,0,0.18); border-radius:6px; font-family:var(--mono); font-size:10px; color:#ffb300; }
        .ws-badge-dot { width:5px; height:5px; border-radius:50%; background:#ffb300; animation:dotPulse 2s ease-in-out infinite; }
        ...
        .btn-sm-v:hover:not(:disabled) { background:rgba(255,140,0,0.2); border-color:rgba(255,140,0,0.5); }
        .btn-sm-t { background:rgba(255,140,0,0.08); border-color:rgba(255,140,0,0.2); color:#fbbf24; }
        .btn-sm-t:hover:not(:disabled) { background:rgba(255,140,0,0.15); border-color:rgba(255,140,0,0.45); }
        .btn-sm-g { background:rgba(255,140,0,0.08); border-color:rgba(255,140,0,0.2); color:#ff8c00; }
        .btn-sm-g:hover:not(:disabled) { background:rgba(255,140,0,0.15); border-color:rgba(255,140,0,0.45); }
        .btn-sm-a { background:rgba(245,158,11,0.09); border-color:rgba(245,158,11,0.22); color:var(--amber); }
        .btn-sm-a:hover:not(:disabled) { background:rgba(245,158,11,0.18); border-color:rgba(245,158,11,0.5); }
        ...
        .no-ws-warn { margin-top:8px; padding:8px 11px; background:rgba(245,158,11,0.06); border:1px solid rgba(245,158,11,0.18); border-radius:7px; font-family:var(--mono); font-size:10px; color:#fbbf24; line-height:1.6; }
        ...
        .cr-body { padding:8px; }
        .ccard { border-radius:var(--r2); padding:12px 14px; margin-bottom:7px; border:1px solid; animation:upIn 0.4s ease both; transition:all 0.15s; }
        ...
        .rag-send:hover:not(:disabled) { background:rgba(255,140,0,0.22); border-color:rgba(255,140,0,0.5); }
        ...
        .modal-bg { position:fixed; inset:0; z-index:200; background:rgba(0,0,0,0.88); backdrop-filter:blur(22px); display:flex; align-items:center; justify-content:center; padding:24px; animation:fadeIn 0.2s ease; }
        @keyframes fadeIn { from{opacity:0} to{opacity:1} }
        .modal { background:var(--bg2); border:1px solid var(--line2); border-radius:var(--r4); width:100%; max-width:940px; max-height:90vh; overflow:hidden; display:flex; flex-direction:column; box-shadow:0 36px 80px rgba(0,0,0,0.75); animation:modalIn 0.25s ease; }
        ...
        <div className="shell">
          ...
          <nav className="topbar">
            ...
            <div className="topbar-right">
              ...
              <span className="pill pill-t">Multi-lang · Auto-translate</span>
              <span className="pill pill-a">Entities · KG · RAG</span>
          ...
          {clusters && (
            <div className="stats">
              <div className="scard sc-v"><div className="slbl">Total Files</div><div className="sval">{totalFiles}</div><div className="ssub">processed this session</div></div>
              <div className="scard sc-v"><div className="slbl">Clusters</div><div className="sval">{clusterCount}</div><div className="ssub">semantic groups</div></div>
              <div className="scard sc-t"><div className="slbl">Translated</div><div className="sval">{Object.keys(translatedFiles).length}</div><div className="ssub">non-English docs</div></div>
              <div className="scard sc-g"><div className="slbl">RAG Chunks</div><div className="sval">{ragReady ? "✓" : "—"}</div><div className="ssub">{ragReady ? "indexed & ready" : "not yet indexed"}</div></div>
              <div className="scard sc-a"><div className="slbl">Languages</div><div className="sval">{Object.keys(overallLang).length || "—"}</div><div className="ssub">detected across docs</div></div>
            </div>
          )}
                  ...
                  {sidebarTab === "files" && (
                    <>
                      <div className="winput-wrap">
                        <div className="winput-label">
                          <Icon d="M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z" size={9} sw={2} />
                          Workspace ID (optional — leave blank to auto-create)
                        </div>
                        <input
                          className="winput"
                          placeholder="e.g. my-project-2026"
                          value={workspaceInput}
                          onChange={e => setWorkspaceInput(e.target.value)}
                          disabled={loading}
                        />
                        {/* FIX: show active workspace ID after upload */}
                        {workspaceId && (
                          <div className="ws-badge">
                            <div className="ws-badge-dot" />
                            Active workspace: <strong>{workspaceId}</strong>
                          </div>
                        )}
                      </div>
                      ...
                      >
                        {files.length === 0 ? (
                          <>
                            ...
                            <p className="drop-hint">PDFs, DOCX, TXT and more<br />Marathi, Hindi, Arabic — auto-translated to English</p>
                          </>
                        ) : (
                      ...
                      <div className="actions">
                        <button className="btn-primary" onClick={handleUpload} disabled={loading}>
                          {loading ? "Processing…" : "Run Intelligence Pipeline"}
                        </button>
                        <button className="btn-ghost" onClick={() => window.location.reload()}>
                          <Icon d="M1 4v6h6M23 20v-6h-6M20.49 9A9 9 0 005.64 5.64L1 10M23 14l-4.64 4.36A9 9 0 013.51 15" size={12} sw={2} />Reset
                        </button>
                      </div>
                      ...
                      {loading && (
                        <div className="progress-wrap">
                          ...
                          <div className="prog-status"><div className="spin" /><span className="prog-txt">Detecting · Translating · Embedding · Clustering · Indexing RAG…</span></div>
                        </div>
                      )}
                  ...
                  {sidebarTab === "workspace" && (
                    <div>
                      <div className="winput-wrap">
                        ...
                        <input
                          className="winput"
                          placeholder="Leave blank to auto-create on upload"
                          value={workspaceInput}
                          onChange={e => setWorkspaceInput(e.target.value)}
                        />
                      </div>
                      <div style={{ fontSize:11, color:"var(--tx3)", fontFamily:"var(--mono)", marginBottom:10, lineHeight:1.7, background:"rgba(124,58,237,0.05)", border:"1px solid rgba(124,58,237,0.12)", borderRadius:7, padding:"8px 10px" }}>
                        ...
                        2. Go to <span style={{color:"var(--vl)"}}>Files</span> tab → upload → Run Intelligence Pipeline<br/>
                        3. Come back here → Entity extraction will be linked automatically
                      </div>

                      {workspaceId && (
                        <div style={{ background:"rgba(255,140,0,0.04)", border:"1px solid rgba(255,140,0,0.14)", borderRadius:10, padding:"12px 14px", marginBottom:12 }}>
                          <div className="icard-lbl" style={{marginBottom:7}}>Active Session</div>
                          <div style={{ fontSize:11, color:"var(--tx2)", lineHeight:1.8 }}>
                            <div>📁 {totalFiles} documents processed</div>
                            <div>🗂️ {clusterCount} clusters formed</div>
                            <div>🌐 {Object.keys(overallLang).length} languages detected</div>
                            {entities?.total !== undefined && <div>🏷️ {entities.total} entities extracted</div>}
                            {relationships?.total !== undefined && <div>🔗 {relationships.total} relationships found</div>}
                            <div style={{marginTop:6, color:"var(--amber)", fontFamily:"var(--mono)", fontSize:10}}>
                              WS: {workspaceId}
                            </div>
                          </div>
                        </div>
                      ...
                      <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
                        <button className="btn-sm btn-sm-g" onClick={dlReport} disabled={!intelligenceRep}>
                          <Icon d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" size={11} sw={2} />Export Report
                        </button>
                        <button className="btn-sm btn-sm-v" onClick={runEntityExtraction} disabled={intelDisabled}>
                          {entityLoading ? <><div className="spin" style={{width:9,height:9}} />Running…</> : "Extract Entities"}
                        </button>
                        <button className="btn-sm btn-sm-t" onClick={runRelationshipExtraction} disabled={intelDisabled}>
                          {entityLoading ? "Running…" : "Relationships"}
                        </button>
                        <button className="btn-sm btn-sm-a" onClick={buildKnowledgeGraph} disabled={intelDisabled}>
                          {entityLoading ? "Running…" : "Build KG"}
                        </button>
                      </div>

                      {/* FIX: clear guidance when workspace not linked */}
                      {clusters && !workspaceId && (
                        <div className="no-ws-warn">
                          ⚠ Pipeline ran but workspace ID was not captured.<br/>
                          Please reload and re-upload with a Workspace ID to enable entity extraction.
                        </div>
                      )}
                      {!clusters && (
                        <div style={{marginTop:10, fontFamily:"var(--mono)", fontSize:10, color:"var(--tx3)"}}>
                          Upload and process documents first to enable intelligence features.
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {sidebarTab === "drive" && (
                    <div>
                      <div style={{ fontSize:12, color:"var(--tx2)", marginBottom:12, lineHeight:1.6 }}>
                        Ingest documents directly from a Google Drive folder.
                      </div>
                      <div className="drive-panel">
                        <div className="drive-label">Google Drive Folder URL or ID</div>
                        <div className="drive-row">
                          ...
                          <button className="btn-sm btn-sm-a" onClick={ingestDrive} disabled={driveLoading} style={{flexShrink:0}}>
                            {driveLoading ? <div className="spin" style={{borderTopColor:"var(--amber)",borderColor:"rgba(245,158,11,0.2)",width:10,height:10}} /> : "Ingest"}
                          </button>
                        </div>
                        {driveResult && (
                          <div className="drive-result">
                            {driveResult.error
                              ? <span style={{color:"var(--rose)"}}>⚠ {driveResult.error}</span>
                              : <span style={{color:"var(--green)"}}>✓ {driveResult.message || JSON.stringify(driveResult).slice(0,120)}</span>}
                          </div>
                        )}
                      </div>
                  ...
                  {intelligenceRep && (
                    <div className="repbar">
                      <div className="repbar-l">
                        ...
                        <div><div className="rep-lbl">Intelligence Report Ready</div><div className="rep-sub2">{clusterCount} clusters · {totalFiles} documents</div></div>
                      </div>
                      <button className="btn-sm btn-sm-g" onClick={dlReport}>
                        <Icon d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" size={10} sw={2} />Export JSON
                      </button>
                    </div>
                  )}
                </div>
            ...
            <div>
              <div className="rpanel">
                <div className="rpanel-head">
                  ...
                  <div><div className="ph-title">Cluster Analysis & Intelligence</div><div className="ph-sub">Embeddings · Entity extraction · Knowledge graph · RAG chat</div></div>
                </div>
                <div className="rbody">
                  {/* Graphs */}
                  <div className="ggrid">
                    <div>
                      ...
                      <div className="gbox">{elbowGraph ? <img src={elbowGraph} alt="Elbow" /> : <div className="gempty"><div className="gempty-icon"><Icon d="M22 12H18M15 21L9 3 6 12 2 12" size={15} sw={1.5} /></div><span className="gempty-txt">Awaiting analysis</span></div>}</div>
                    </div>
                    <div>
                    ...
                    <>
                      ...
                      <div className="tabs">
                        ...
                        <button className={`tab${activeTab==="intel"?" on-a":""}`} onClick={() => setActiveTab("intel")}>
                          ...
                          Intelligence <span className="tc">KG</span>
                        </button>
                        <button className={`tab${activeTab==="chat"?" on-g":""}`} onClick={() => setActiveTab("chat")}>
                          ...
                          Doc Chat {ragReady && <span className="tc">RAG</span>}
                        </button>
                      </div>
                          ...
                          {Object.entries(summaries).map(([name, summary], idx) => {
                            ...
                            return (
                              <div key={name} className="sc2" style={{ background:col.bg, borderColor:col.border }}>
                                ...
                                {summary?.length > 150 && <button className="expbtn" onClick={() => setExpandedSummaries(p => ({...p,[name]:!p[name]}))}>{exp?"▲ less":"▼ more"}</button>}
                              </div>
                            );
                          })}
                        </div>
                        ...
                        <div className="ins-wrap">
                          ...
                          {activeInsight && (
                            <div className="ins-detail">
                              <div className="icard"><div className="icard-lbl">Auto Summary</div><div className="icard-val">{activeInsight.summary || "—"}</div></div>
                              ...
                              {keywords[activeInsight.cluster_name] && (
                                <div className="icard"><div className="icard-lbl">Smart Tags · Top Keywords</div><div className="kw-wrap">{keywords[activeInsight.cluster_name].map(k => <span key={k} className="kw-chip">{k}</span>)}</div></div>
                              )}
                              {repDocs[activeInsight.cluster_name] && (
                                <div className="icard">
                                  <div className="icard-lbl">Document Importance Ranking</div>
                                  <div className="rep-doc-row"><span>⭐</span><span className="rep-doc-name">{repDocs[activeInsight.cluster_name].most_representative}</span><span className="rep-badge">Top Rep.</span></div>
                      ...
                      {activeTab === "viz" && (
                        <div className="vizp">
                          <div className="viz-hdr">
                            <div><div className="viz-title2">Semantic Cluster Map</div><div className="viz-sub2">PCA — 2D projection of Sentence-BERT embeddings</div></div>
                          </div>
                          {vizData.length > 0 ? (
                            <>
                            ...
                            <div className="gempty" style={{height:200}}><div className="gempty-icon"><Icon d="M12 12m-3 0a3 3 0 106 0 3 3 0 00-6 0" size={15} sw={1.5} /></div><span className="gempty-txt">Run analysis to see visualization</span></div>
                          )}
                        </div>
                      )}
                              ...
                              <div key={cname} className="icard" style={{borderColor:col.border}}>
                                ...
                                <div className="lang-dist">
                                  {Object.entries(dist).map(([lang,cnt]) => (
                                    <div key={lang} className="lang-item"><span className="lang-code">{lang.toUpperCase()}</span><span className="lang-cnt">{cnt} · {Math.round(cnt/total*100)}%</span></div>
                                  ))}
                                </div>
                              </div>
                            );
                        ...
                        <div>
                          <div className="intel-grid">
                            <div className="intel-action">
                              ...
                              <button className="btn-sm btn-sm-v" onClick={runEntityExtraction} disabled={intelDisabled}>
                                {entityLoading ? <><div className="spin" style={{width:10,height:10}} />Running…</> : "Extract Entities"}
                              </button>
                            </div>
                            <div className="intel-action">
                              ...
                              <button className="btn-sm btn-sm-t" onClick={runRelationshipExtraction} disabled={intelDisabled}>
                                {entityLoading ? "Running…" : "Detect Relationships"}
                              </button>
                            </div>
                            <div className="intel-action">
                            ...
                            <div className="intel-action">
                              ...
                              <button className="btn-sm btn-sm-g" onClick={dlReport} disabled={!intelligenceRep}>
                                <Icon d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" size={11} sw={2} />Download Report
                              </button>
                            </div>
                          </div>

                          {/* FIX: show warning if buttons disabled due to missing workspace */}
                          {clusters && !workspaceId && (
                            <div className="no-ws-warn" style={{marginBottom:12}}>
                              ⚠ No workspace linked to this session. Reload and re-upload with a Workspace ID set to enable entity/relationship extraction.
                            </div>
                          )}
                              ...
                              {knowledgeGraph && !knowledgeGraph.error && (
                                <div className="kg-stats-row" style={{padding:"12px 14px 0"}}>
                                  <div className="kg-stat">
                                    ...
                                    <div className="kg-stat-val" style={{color:"var(--vl)"}}>{knowledgeGraph.stats?.total_nodes ?? knowledgeGraph.nodes?.length ?? "—"}</div>
                                  </div>
                                  <div className="kg-stat">
                                    <div className="kg-stat-lbl">Edges</div>
                                    <div className="kg-stat-val" style={{color:"var(--teal)"}}>{knowledgeGraph.stats?.total_edges ?? knowledgeGraph.edges?.length ?? "—"}</div>
                                  </div>
                                  <div className="kg-stat">
                          ...
                          {entities && (
                            <div className="entity-box">
                              ...
                              {entities.error ? (
                                ...
                                if (!Object.keys(typeMap).length) return <div style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--tx3)"}}>No entities found.</div>;
                              ...
                              {relationships.error ? (
                                ...
                                return rels.slice(0,15).map((rel,i) => (
                                  <div key={i} className="rel-item">
                                    ...
                                    <span className="rel-pred">{rel.relationship||rel.predicate||"→"}</span>
                          ...
                          {!entities && !relationships && !knowledgeGraph && (
                            <div className="intel-empty">
                              ...
                              <div className="intel-empty-txt">
                                Run entity extraction, relationship detection, or build a knowledge graph.<br />
                                {!workspaceId ? "Upload and process documents first — workspace ID is required." : "Use the buttons above to begin."}
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="rag-wrap">
                          <div className="rag-hdr">
                            ...
                            <div className="rag-hdr-info">
                              ...
                              <div className="rag-hdr-sub">{ragReady ? "RAG index ready — semantic search + LLM generation" : "Upload and process documents to enable chat"}</div>
                            </div>
                          ...
                          {ragReady && clusters && (
                            <div className="rag-ctrl">
                              ...
                              <div className="rag-topk">
                                ...
                                <button className="rag-kb" onClick={() => setChatTopK(k => Math.max(1,k-1))}>−</button>
                                  ...
                                  {chatMessages.map((msg, i) => (
                                    <div key={i} className={`rag-msg ${msg.role}`}>
                                      ...
                                      <div className="rag-bw">
                                        <div className={`rag-bubble ${msg.role==="assistant" ? (msg.isWelcome ? "ai wel" : "ai") : "usr"}`}>{renderMsg(msg.content)}</div>
                                        {msg.role==="assistant" && msg.sources?.length > 0 && (
                                          <>
                                            <button className={`src-toggle${expandedSources[i]?" open":""}`} onClick={() => setExpandedSources(p=>({...p,[i]:!p[i]}))}>
                                              ...
                                              {msg.cluster_scope && <span style={{opacity:0.55}}> · {cap(msg.cluster_scope)}</span>}
                                            </button>
                                            {expandedSources[i] && (
                                                ...
                                                {msg.sources.map((src,si) => (
                                                  <div key={si} className="rag-src">
                                                    ...
                                                    <div className="rag-src-body">
                                                      ...
                                                      <div className="rag-src-meta">{cap(src.cluster_name)} · {src.language?.toUpperCase()}</div>
                          ...
                          {ragReady && (
                            <div className="rag-ia">
                              <div className="rag-irow">
                                <textarea ref={chatInputRef} className="rag-ta" rows={1} placeholder="Ask anything about your documents…" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => { if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendChat();} }} disabled={chatLoading} />
                              ...
                              <div className="rag-hint">Enter to send · Shift+Enter for newline</div>
                            </div>
                          )}
                        </div>
                      )}
            ...
            <div className="modal">
              <div className="modal-bar">
                ...
                <div className="modal-acts">
                  {activeFileName && <button className="btn-sm btn-sm-v" onClick={e=>dlOrig(e,activeFileName)}>Original</button>}
                  {activeFileName&&hasTrans(activeFileName) && <button className="btn-sm btn-sm-t" onClick={e=>dlTrans(e,activeFileName)}>Translated</button>}
              ...
              {viewingTranslated && (
                <div className="modal-tr-bar">
                  ...
                  <span className="tr-txt">Viewing translated version · {activeFileName&&getLangTag(activeFileName)?getLangTag(activeFileName):"→EN"}</span>
                </div>
              )}
