...
export const CelestialSphere: React.FC<CelestialSphereProps> = ({
  ...
  useEffect(() => {
    ...
    let scene: THREE.Scene, camera: THREE.OrthographicCamera, renderer: THREE.WebGLRenderer, material: THREE.ShaderMaterial, mesh: THREE.Mesh, geometry: THREE.PlaneGeometry;
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Reduced brightness/intensity so it acts as a subtle background
        vec3 color = hsl2rgb(vec3(u_hue / 360.0 + nebula * 0.1, 0.8, 0.4));
        color *= nebula * 1.5;
        ...
        if (star_val > 0.998) {
            ...
            color += vec3(star_brightness * u_particle_size * 0.5);
        }
